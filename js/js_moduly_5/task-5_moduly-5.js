...
// - Добавь `статический` метод `getSpecs(car)`, который принимает объект-машину как 
  // параметр и возвращает шаблонную строку со свойствами и значениями объекта.
  //  - Свойства: maxSpeed, speed, isOn, distance, price
  //  - Пример строки, полученной этим методом:
  //  - 'maxSpeed: 200, speed: 50, isOn: true, distance: 100, price: 2000'
   
  //  Конструктор получает объект настроек.
  //   Добавь свойства будущеего экземпляра класса: speed - текущая скорость, начальное
  //  значение `0`, price - цена автомобиля, maxSpeed - максимальная скорость
  ...
  //  цены автомобиля. ВАЖНО: для записи методов get и set значение параметра записывают с 
  ...
  //  Метод, чтобы заглушить автомобиль Этот метод должен записывать в свойство isOn 
  // значение false, и сбрасывать текущую скорость до 0
  //  turnOff() {}
...
//    Поэтому используют прием смены названия добавляя символ '_'. Хотя это может
//     любой другой символ
//   this._name = name;
//   // }
...
class Car {
 // Write code under this line
  ...
  // { console.log(`maxSpeed: ${Car.maxSpeed}, speed: ${mustang.speed}, isOn:${this.isOn}, distance:${this.distance}, price:${this.price}`); }
    // getSpecs(){}
  // constructor(){}
  ...
  set price(value) { this._price = value;}
    ...
    turnOff() {
      ...
      this.speed = 0;}
  accelerate(value) {
    if (this.speed + value < this.maxSpeed) { this.speed = this.speed + value }
    else {this.speed = this.maxSpeed}}
  decelerate(value) {
    // if (this.speed > 0) { this.speed = this.speed - value }
    // if (this.speed - value < 0) { this.speed = 0 };
    if (this.speed - value > 0) { this.speed = this.speed - value }
      else { this.speed = 0 }
  }
  drive(hours) {if (this.isOn === true) {this.distance = this.distance+(hours * this.speed)}}
}
